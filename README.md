...
- 日出、日落時，朝向太陽的地平線與遠景霧增加暖色光暈；正午、深夜淡出，雨天減弱。保留原版日月與星空。
- 可調海拔的低處薄霧，在晨昏與下雨時較明顯；24 格內不增加高度霧，高處與深地下逐漸淡出，暗處降低霧量。
...
新增大氣效果排除手持物、地獄／終界／自訂維度，以及水下、岩漿、細雪中的相機。
高度霧以海拔、場景亮度與深度近似，無法判斷每個洞穴或屋頂是否真的露天；高處有光的室內仍可能出現少量遠景霧。
`FOG_STRENGTH` 控制原有遠景霧；若要關閉全部額外霧，還需把 `HEIGHT_FOG` 設為 0。
...
| Sunrise and sunset glow | 0.35 | 0.65 |
| Low-altitude morning and rain mist | 0.25 | 0.5 |
| Mist reference altitude (world Y) | 64 | 依地形選 32／64／96／128 |
